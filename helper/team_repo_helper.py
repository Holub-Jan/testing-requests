...
class TeamRepositoryHelper(GenericHelper):
    ...
    def get_role(self, query: List[Tuple]):
        team_repo = self._team_repo_storage.select_by_query(query)
        if team_repo:
            return team_repo[0].role

    def get_id(self, name: str, team_id: int, repo_id: int, role: str):
        query = [('team_id', team_id), ('repo_id', repo_id)]
        team_repo_exists = self.exists(query)
        if team_repo_exists:
            team_repo_obj = self.get_or_create(name, team_id, repo_id, role)
